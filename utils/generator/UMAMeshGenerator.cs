...
namespace UMA
{
    public class UMAMeshGenerator
    {
        ...
        public Skin generatedSkin;
        public ArrayMesh generatedMesh;

        public void generate(ArrayMesh oldMesh, Skin skin, UMAReciepe reciepe, List<string> boneNames)
        {
            //reset changes value
            changes = 0;
        ...
        */
        ...
        private List<UMAReciepeBindPose> extractBinds(string urlPath)
        {
            ...
            {
                foreach (var child in (instance as Node3D).GetChildren())
                {
                    if (child is MeshInstance3D)
                        meshes.Add(child as MeshInstance3D);
                }
            }

            if (instance is MeshInstance3D)
            {
                meshes.Add(instance as MeshInstance3D);
            }
        ...
        private ArrayMesh cleanOldOverlays(ArrayMesh oldMesh, UMAReciepe reciepe)
        {
            ...
            foreach (var oldSurface in oldSurfaces)
            {
                ...
                if (!exist)
                {
                    ...
                    oldMesh._Surfaces.Remove(oldMesh.SurfaceFindByName(oldSurface));
                    changes++;
                }
        ...
        */
        public ArrayMesh createSurfaceOverlay(ArrayMesh mesh, string slotName, string urlPath, string overlayName, List<UMAReciepeBindPose> origBindPoses)
        {
            ...
            {
                var newMesh = (mi as MeshInstance3D).Mesh;
                var newSkin = (mi as MeshInstance3D).Skin;
                ...
                for (int surface = 0; surface < newMesh.GetSurfaceCount(); surface++)
                {
                    ...
                    {
                        (mat as StandardMaterial3D).Grow = true;
                        (mat as StandardMaterial3D).GrowAmount = 0.005f;
                    }
        ...
        private Godot.Collections.Array createSurfaceByBones(ArrayMesh mesh, int surface, Skin newSkin, List<UMAReciepeBindPose> origBindPoses)
        {
            ...
            for (int i = 0; i < mdt.GetVertexCount(); i++)
            {
                ...
                st.SetBones(mdt.GetVertexBones(i));
                st.SetWeights(mdt.GetVertexWeights(i));
                ...
                st.SetUv(mdt.GetVertexUv(i));

                if (mdt.GetVertexColor(i) != null)
                    st.SetColor(mdt.GetVertexColor(i));

                if (mdt.GetVertexUv2(i) != null)
                    st.SetUv2(mdt.GetVertexUv2(i));

                st.SetNormal(newNorm);
                st.AddVertex(newVer);
